...
def setTime():
    ...
    if direction=="right":
        last_data[0]=totalVehiclesLane1=right_model.predict(np.array([itern]).reshape(-1,1))[0][0]-last_data[0]
        last_data[1]=totalVehiclesLane2=down_model.predict(np.array([itern]).reshape(-1,1))[0][0]-last_data[1]
        last_data[2]=totalVehiclesLane3=left_model.predict(np.array([itern]).reshape(-1,1))[0][0]-last_data[2]
        last_data[3]=totalVehiclesLane4=right_model.predict(np.array([itern]).reshape(-1,1))[0][0]-last_data[3]
